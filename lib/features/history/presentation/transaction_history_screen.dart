      ...
      body: CustomScrollView(
        ...
        slivers: [
          SliverAppBar(
            ...
            actions: [
              IconButton(
                icon: const Icon(LucideIcons.search, color: AppColors.e8, size: 20),
                onPressed: () {},
              ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // Summary Cards
                  _HistorySummary(totalIngresos: totalIngresos, totalGastos: totalGastos, fmt: _fmt)
                      .animate()
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text("No hay movimientos en esta categoría", style: TextStyle(fontSize: 14, color: AppColors.g5)),
        ],
      ),
    );
  }
...
class _HistorySummary extends StatelessWidget {
  ...
  const _HistorySummary({required this.totalIngresos, required this.totalGastos, required this.fmt});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: AppColors.e8.withValues(alpha: 0.25), blurRadius: 25, offset: const Offset(0, 12))
        ],
      ),
      child: Row(
        children: [
...
class _SummaryItem extends StatelessWidget {
  ...
  const _SummaryItem({required this.label, required this.amount, required this.color, this.isRight = false});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.white.withValues(alpha: 0.4), letterSpacing: 0.5)),
          const SizedBox(height: 4),
          Text(amount, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: color, letterSpacing: -0.5)),
        ],
      ),
    );
  }
...
class _HistoryFilters extends StatelessWidget {
  ...
  const _HistoryFilters({required this.filters, required this.selectedFilter, required this.onChanged});
...
class _DayGroupSection extends StatelessWidget {
  ...
  const _DayGroupSection({required this.dateKey, required this.dayTotal, required this.dayTxns, required this.activeBudget, required this.wallets, required this.fmt});
        ...
        children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text(dateKey, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: AppColors.e8, letterSpacing: -0.3)),
                if (dayTotal > 0)
                  Text("- ${fmt(dayTotal)}", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.g4)),
              ],
            ),
          ),
          Container(
            ...
            child: Column(
              children: List.generate(dayTxns.length, (i) {
                ...
                final fromW = t.fromAccountId != null ? wallets.where((w) => w.id == t.fromAccountId).firstOrNull : null;
                final toW = t.toAccountId != null ? wallets.where((w) => w.id == t.toAccountId).firstOrNull : null;
...
class _HistoryTile extends StatelessWidget {
  ...
  const _HistoryTile({required this.transaction, this.category, required this.isTransfer, this.fromWallet, this.toWallet, required this.isLast, required this.fmt});
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (isTransfer && fromWallet != null && toWallet != null)
                        Text("${fromWallet!.nombre.split('—').first.trim()} → ${toWallet!.nombre.split('—').first.trim()}", style: const TextStyle(fontSize: 11, color: AppColors.g4, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)
                      else
                        Text(category?.label ?? transaction.catKey.toUpperCase(), style: const TextStyle(fontSize: 11, color: AppColors.g4, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ),
                const SizedBox(width: 12),
                Text(
                  isTransfer ? fmt(transaction.monto.abs()) : (transaction.tipo == "ingreso" ? "+${fmt(transaction.monto.abs())}" : "-${fmt(transaction.monto.abs())}"),
                  style: TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.w900,
                    color: isTransfer ? AppColors.b5 : (transaction.tipo == "ingreso" ? AppColors.e6 : AppColors.e8),
                  ),
                ),
              ],
            ),
          ),
          if (!isLast) Divider(height: 1, color: AppColors.g1, indent: 76, endIndent: 18),
        ],
      ),
    );
  }
